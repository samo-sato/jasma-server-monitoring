...
import { toInt, generateRandomString, msToWords, serErr, validateEnv } from '../../src/utils.js';
    ...
    INSERT INTO Watchdog (
      ...
      $enabled
        ...
        const params = {
            $id: wid,
            $uuid: uuid,
            $url: data.passive === 1 ? null : data.url,
            $name: data.name,
            $email_notif: data.email_notif,
            $threshold: data.threshold,
            ...
            if (error) {
                ...
                const convertedRows = rows.map((row) => {
                    if (typeof row === 'object' && row !== null) {
                        return Object.assign(Object.assign({}, row), { datetime: getCurrentDateTimeUTC(row.timestamp) });
                    }
                    return row;
                });
                resolve({
