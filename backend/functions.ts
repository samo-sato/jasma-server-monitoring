 ...
 */
export function generateActivationKeyUrl(): KeyUrl {
 ...
 */
export function generateUnsubscribeKeyUrl(optionalKey?: string): KeyUrl {
...
*/
export function handleError(error: any, generalMsg: string): number {
 ...
 */
export function saltedPwHash(password: string, salt: string): string {
 ...
 */
export function getCurrentDateTimeUTC(customTimestamp?: number): string {
 ...
 */
export function generateWatchdogId(): string {
...
function createTransportOptions(): SMTPTransport.Options {
  ...
  if ( JASMA_MAIL_SECURE.toLowerCase() === 'true' ) { transportOptions.secure = true }
  if ( JASMA_MAIL_SECURE.toLowerCase() === 'false') { transportOptions.secure = false }
  ...
  if ( JASMA_MAIL_TLS_REJECT_UNAUTHORIZED.toLowerCase() === 'false') { rejectUnauthorized = false }
  if ( JASMA_MAIL_TLS_REJECT_UNAUTHORIZED.toLowerCase() === 'true' ) { rejectUnauthorized = true }
 ...
 */
function createMailOptions(recipient: string, subject: string, message: string): SMTPTransport.Options {
  ...
  if (fromAddress || fromName) {
    mailOptions.from = {
      address: fromAddress ?? '',  // Use an empty string as a fallback if address is undefined
      name: fromName,
    };
  }
 ...
 * @returns Promise
 */
export function sendMail(recipient: string, subject: string, message: string, activation?: boolean): Promise<SendMailResponse> {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const unsubKeyResult = await isNotUnsubscribed(recipient)
      let unsubKey: string = unsubKeyResult.data
  ...
  Thank you for using ${urlBase(urlBaseEnvs, false)} for your server monitoring needs.`
      ...
      await transporter.sendMail(createMailOptions(recipient, subject, message));
